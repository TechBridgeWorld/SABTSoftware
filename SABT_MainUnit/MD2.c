	...
	switch (next_state) {
		...
		case STATE_PROMPT:
			if (incorrect_tries == MAX_INCORRECT_TRIES) {
				sprintf(dbgstr, "[MD2] Hit incorrect try threshold\n\r");
				PRINTF(dbgstr);
				incorrect_tries = 0;
		...
		case STATE_INPUT:
			...
			if (get_character(&user_glyph) == true) {
				PRINTF("[MD2] Valid character received\n\r");
		...
		case STATE_CHECK:
			if (glyph_equals(curr_glyph, user_glyph)) {
				PRINTF("[MD2] User answered correctly\n\r");
				incorrect_tries = 0;
				play_mp3(LANG_FILESET, MP3_CORRECT);
				play_mp3(SYS_FILESET, MP3_TADA);
				next_state = STATE_GENQUES;
			} else {
				PRINTF("[MD2] User answered incorrectly\n\r");
				incorrect_tries++;
