...
void md9_generate_question(void) {
    ...
    do {
        ...
        log_msg("[MD9] Operand 1: %d", md_op_1);
        
        md_op_2 = timer_rand() % bound;
        log_msg("[MD9] Operand 2: %d", md_op_2);
    ...
    switch (md_next_state) {
        ...
        case STATE_MENU:
            ...
            switch (md_last_dot) {
                ...
                default:
                    log_msg("[MD9] Error: Invalid dot: %c", md_last_dot);
                    quit_mode();
                    break;
            }
            break;
        ...
        case STATE_LVLSEL:
            ...
            switch (md_last_dot) {
                ...
                default:
                    log_msg("[MD9] Error: md_last_dot: %c", md_last_dot);
                    quit_mode();
                    break;
            }
            break;
        ...
        case STATE_INPUT:
            ...
            if (get_number(&md_input_valid, &md_usr_res)) {
                if (md_input_valid) {
                    log_msg("[MD9] User answer: %d", md_usr_res);
        ...
        case STATE_CHECKANS:
            if (md_usr_res == md_res) {     // Correct answer
                md_incorrect_tries = 0;
