...
//Mode states
#define MD8_STATE_MENU			0x00
#define MD8_STATE_INIT 			0x01
#define MD8_STATE_PROMPT		0x02
#define MD8_STATE_INPUT			0x03
#define MD8_STATE_CHECK 		0x04
#define MD8_STATE_NEXT 			0x05
#define MD8_STATE_INCORRECT 0x06
#define MD8_STATE_TRYAGAIN 	0x07
#define MD8_STATE_SKIP			0x08
#define MD8_STATE_START			0x09
#define MD8_STATE_PREV			0x0B
...
#define MD8_SUBMODE_NONE		0x00
#define MD8_SUBMODE_LEARN		0x01
#define MD8_SUBMODE_PLAY		0x02
...
#define SCRIPT_LENGTH SCRIPT_COMMON_LENGTH
...
static script_t* this_script = &script_common;
static char this_lang_fileset[5] = "ENG_";
static char this_mode_fileset[5] = "MD8_";

/* Defines initial state for mode state machine */
static char next_state = MD8_STATE_START;
...
void md8_main(void) {
	...
	switch(next_state) {
		...
		// Initialises mode
		case MD8_STATE_START:
			PRINTF("*** MD8 - Number practice ***\n\r");
			PRINTF("MD8_STATE_START\n\r");
			md8_reset_vars();
			next_state = MD8_STATE_MENU;
			break;
		...
		case MD8_STATE_MENU:
			...
			switch (last_dot) {
				...
				case '1':
					submode = MD8_SUBMODE_LEARN;
					PRINTF("** LEARN submode **\n\r");
					next_state = MD8_STATE_INIT;
					last_dot = 0;
					incorrect_tries = 0;
					break;

				case '2':
					submode = MD8_SUBMODE_PLAY;
					PRINTF("** PLAY submode **\n\r");
					next_state = MD8_STATE_INIT;
					last_dot = 0;
					incorrect_tries = 0;
		...
		case MD8_STATE_PROMPT:
			
			PRINTF("MD8_STATE_PROMPT\n\r");
			switch (submode) {
				case MD8_SUBMODE_LEARN:
		...
		case MD8_STATE_INPUT:
			...
			switch (last_dot) {
				...
				case ENTER:
					next_state = MD8_STATE_CHECK;
					break;

				case LEFT:
					play_mp3(mode_fileset, "NEXT");
					next_state = MD8_STATE_PREV;
					break;
		...
		case MD8_STATE_CHECK:
			PRINTF("MD8_STATE_CHECK\n\r");
		...
		case MD8_STATE_INCORRECT:
			PRINTF("MD8_STATE_INCORRECT\n\r");
			//After a certain number of incorrect answers, ask user if they want
			// to skip
			if (incorrect_tries >= MAX_INCORRECT_TRIES) {
				next_state = MD8_STATE_SKIP;
			} else {
				next_state = MD8_STATE_PROMPT;
			}
			break;
